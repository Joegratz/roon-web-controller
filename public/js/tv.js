...
$(document).ready(function() {
    showPage();
    fixFontSize();
	$("#containerZoneList, #controlsPlayer, #controlsSettings").hide();
});

var i = null;
$(document).mousemove(function() {
    clearTimeout(i);
    $("#containerZoneList, #controlsPlayer, #controlsSettings").slideDown();
    i = setTimeout(function () {
        $("#containerZoneList, #controlsPlayer, #controlsSettings").slideUp();
    }, 5000);
}).mouseleave(function() {
    clearTimeout(i);
    $("#containerZoneList, #controlsPlayer, #controlsSettings").slideUp();  
...
function fixFontSize() {
    $(".lineMusicInfo").css('font-size', Math.round(parseInt($("#line1").height() * .9)));
	...
	var trackSeekValueFontSize = $("#line1").height() * .5;
	$("#trackSeekValue span").css('font-size', trackSeekValueFontSize);
	$("#trackSeekValue span").css('bottom', trackSeekValueFontSize * -1.5);
	$(document.body).css('font-size', trackSeekValueFontSize);
...
function showIsPlaying(curZone) {
    ...
    if ( state.line1 != curZone.now_playing.three_line.line1) {
		...
		curZone.now_playing.three_line.line1 = prettifyCharacters(curZone.now_playing.three_line.line1);
		fixFontSize();
        $("#line1")
    ...
    if ( state.line2 != curZone.now_playing.three_line.line2) {
		...
		curZone.now_playing.three_line.line2 = prettifyCharacters(curZone.now_playing.three_line.line2);
		$("#line2")
    ...
    if ( state.line3 != curZone.now_playing.three_line.line3) {
		...
		curZone.now_playing.three_line.line3 = prettifyCharacters(curZone.now_playing.three_line.line3);
        $("#line3")
    ...
    if ( curZone.is_seek_allowed === true ) {
		...
		var previousSeekSliderPercent = $("#trackSeekValue").width() / $("#trackSeek").width();
		var newSeekSliderPercent = (curZone.now_playing.seek_position + 1) / (curZone.now_playing.length + 1);
		// console.log(curZone.now_playing.seek_position + "/" + curZone.now_playing.length);
		// console.log(newSeekSliderPercent);
		// If the seek is backwards or the seek is forwards more than 5%+1s worth, turn off animation; else turn on animation
		if (newSeekSliderPercent + 0.01 < previousSeekSliderPercent) {
			$("#trackSeekValue").css("transition", "width 0s");
			console.log("Jumped back");
		}
		else if (newSeekSliderPercent > (previousSeekSliderPercent + 5/curZone.now_playing.length)) {
			$("#trackSeekValue").css("transition", "width 0s");
			console.log("Jumped forward");
		}
		else {
			$("#trackSeekValue").css("transition", "width 2s linear");
		}
        $("#trackSeekValue").css("width", newSeekSliderPercent * 100 + "%");
    } else {
    ...
    if ( state.image_key != curZone.now_playing.image_key || state.image_key === undefined) {
        ...
        if ( curZone.now_playing.image_key === undefined ) {
            ...
            state.imgUrl = "/roonapi/getOriginalImage?image_key=" + curZone.now_playing.image_key;
        }
...
function prettifyCharacters(string) {
	string = string
		.replace(/\u0027/gi, "\u2019") // apostrophe
		.replace(/"\b/g, "\u201c") // opening double
		.replace(/\b"/g, "\u201d") // closing double
		.replace(/(\d)(\s*-\s*)(\d)/g, "$1\u2013$3") // en dashes between number ranges
		.replace(/(\D)(\s+-+\s+)(\D)/g, "$1\u2014$3") // any number of hyphens surrounded by white space and non-digits to em dashes
		.replace(/\s*-{2,}\s*/g, "\u2014"); // two or more hyphens to em dashes regardless of context, removing whitespace
	return string;
}
